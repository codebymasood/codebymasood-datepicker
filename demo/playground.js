...
(() => {
  ...
  function generateCode() {
    ...
    lines.push(hk('const') + ' dp = ' + hk('new') + ' ' + hf('CbmDatepicker') + '(' + hs(target) + ', {');
    rawLines.push(`const dp = new CbmDatepicker('${target}', {`);
  ...
  function recreate() {
    ...
    if (opts.inline) {
      ...
      pgInstance = new CbmDatepicker('#pgInlineHost', opts);
    } else {
      const wrap = document.createElement('div');
      wrap.className = 'pg-popover-wrap';
      wrap.innerHTML = `
        <div class="cbm-input-wrapper">
          <input class="cbm-input" id="pgPopoverInput" type="text" placeholder="\u06F1\u06F4\u06F0\u06F4/\u06F0\u06F1/\u06F0\u06F1" autocomplete="off">
          <span class="cbm-input-icon">\uD83D\uDCC5</span>
        </div>
      `;
      previewBox.appendChild(wrap);
      previewBox.classList.add('active');
      pgInstance = new CbmDatepicker('#pgPopoverInput', opts);
    }

    if (pgInstance && pgInstance.engine) {
      pgInstance.engine.on('viewChange', (info) => {
        logEvent('viewChange', `${info.monthName} ${CbmEngine.toPersianNum(info.year)} (${info.viewMode})`, info);
      });
    }
  ...
  document.querySelectorAll('.pg-theme-card').forEach(card => {
    card.addEventListener('click', () => {
      ...
      if (theme === 'modern') {
        ...
        document.documentElement.setAttribute('data-cbm-theme', 'classic');
  ...
  document.getElementById('pgHlpBuildRun').addEventListener('click', () => {
    try {
      ...
      const result = CbmEngine.buildDatePayload(y, m, d, fmt);
  ...
  document.getElementById('pgHlpFmtRun').addEventListener('click', () => {
    try {
      ...
      if (isNaN(y) || isNaN(m) || isNaN(d)) throw new Error('Enter valid jy, jm, jd');
      const result = CbmEngine.formatPersian(y, m, d);
  ...
  document.getElementById('pgHlpNumRun').addEventListener('click', () => {
    ...
    const result = CbmEngine.toPersianNum(val);
  ...
  document.getElementById('pgHlpFromRun').addEventListener('click', () => {
    ...
    const result = CbmEngine.fromPersianNum(val);
