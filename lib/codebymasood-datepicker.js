...
/* ================================================================
   cbm ENGINE — Headless Calendar State Manager
   ================================================================ */
class CbmEngine {
  static MONTH_NAMES = [
  ...
  // ── Navigation ──
  goToNextMonth() {
    if (this.viewYear === CbmEngine.MAX_YEAR && this.viewMonth === 12) return;
  ...
  goToPrevMonth() {
    if (this.viewYear === CbmEngine.MIN_YEAR && this.viewMonth === 1) return;
    if (this.viewMonth === 1) {
      this.viewMonth = 12;
      this.viewYear--;
    } else {
  ...
  goToNextYear() {
    if (this.viewYear === CbmEngine.MAX_YEAR) return;
    this.viewYear++;
    this._clampView();
    this.emit('viewChange', this.getViewInfo());
  }
  ...
  goToPrevYear() {
    if (this.viewYear === CbmEngine.MIN_YEAR) return;
    this.viewYear--;
    this._clampView();
    this.emit('viewChange', this.getViewInfo());
  }
  ...
  goToToday() {
    ...
    if (!this.rangeMode) {
      ...
      this.emit('select', CbmEngine.buildDatePayload(this.today.jy, this.today.jm, this.today.jd, this.outputFormat));
    }
    this.emit('viewChange', this.getViewInfo());
  }
  ...
  _clampView() {
    if (this.viewYear < CbmEngine.MIN_YEAR) this.viewYear = CbmEngine.MIN_YEAR;
    if (this.viewYear > CbmEngine.MAX_YEAR) this.viewYear = CbmEngine.MAX_YEAR;
  ...
  selectDate(jy, jm, jd) {
    ...
    if (this.rangeMode) {
      if (!this.rangeStart || this.rangeEnd) {
        ...
        this.rangeEnd = null;
        this.emit('rangeStart', CbmEngine.buildDatePayload(jy, jm, jd, this.outputFormat));
      } else {
        // Set end
        ...
        this.emit('rangeSelect', {
          start: CbmEngine.buildDatePayload(this.rangeStart.jy, this.rangeStart.jm, this.rangeStart.jd, this.outputFormat),
          end:   CbmEngine.buildDatePayload(this.rangeEnd.jy,   this.rangeEnd.jm,   this.rangeEnd.jd,   this.outputFormat),
        });
      }
    } else {
      ...
      this.viewYear = jy;
      this.viewMonth = jm;
      this.emit('select', CbmEngine.buildDatePayload(jy, jm, jd, this.outputFormat));
    }
  }
  ...
  getMonths() {
    return CbmEngine.MONTH_NAMES.map((name, i) => ({
      index: i + 1,
      name,
  ...
  getYears() {
    ...
    if (startYear < CbmEngine.MIN_YEAR) startYear = CbmEngine.MIN_YEAR;
    if (startYear + 11 > CbmEngine.MAX_YEAR) startYear = Math.max(CbmEngine.MIN_YEAR, CbmEngine.MAX_YEAR - 11);
    const years = [];
    for (let i = 0; i < 12; i++) {
      const y = startYear + i;
      years.push({
  ...
  getViewInfo() {
    return {
      ...
      monthName: CbmEngine.MONTH_NAMES[this.viewMonth - 1],
  ...
  static buildDatePayload(jy, jm, jd, format = 'both') {
    ...
    const jalaliPart = {
      ...
      month:           jm,
      day:             jd,
      monthName:       CbmEngine.MONTH_NAMES[jm - 1],
      formatted:       `${jy}/${String(jm).padStart(2,'0')}/${String(jd).padStart(2,'0')}`,
      formattedPersian: CbmEngine.formatPersian(jy, jm, jd),
      timestamp,
    };
  ...
  static parseDateString(str) {
    const normalized = CbmEngine.fromPersianNum(str).replace(/[\/\-\.]/g, '/');
...
/* ================================================================
   cbm RENDERER — Binds Engine to DOM
   ================================================================ */
class CbmRenderer {
  ...
  _renderDayView(info) {
    ...
    let html = `
      <div class="cbm-calendar-header">
        <button class="cbm-nav-btn" data-action="prevMonth">▶</button>
        <div class="cbm-header-title">
          <span class="cbm-title-chip" data-action="showMonth">${CbmEngine.MONTH_NAMES[info.month - 1]}</span>
          <span class="cbm-title-chip" data-action="showYear">${CbmEngine.toPersianNum(info.year)}</span>
        </div>
        <button class="cbm-nav-btn" data-action="nextMonth">◀</button>
      </div>
      <div class="cbm-weekdays">
        ${CbmEngine.WEEKDAY_NAMES.map((name, i) =>
          `<div class="cbm-weekday${i === 6 ? ' weekend' : ''}">${name}</div>`
        ).join('')}
      </div>
      <div class="cbm-days">
        ${days.map(day => {
          const classes = ['cbm-day'];
          ...
          return `<div class="${classes.join(' ')}" data-jy="${day.jy}" data-jm="${day.jm}" data-jd="${day.jd}">${CbmEngine.toPersianNum(day.jd)}</div>`;
        }).join('')}
      </div>
      <div class="cbm-footer">
        ${this.engine.rangeMode
          ? `<span class="cbm-range-hint">${
          ...
          : '<button class="cbm-footer-btn today-btn" data-action="goToday">امروز</button>'
        }
        <button class="cbm-footer-btn clear-btn" data-action="clear">پاک کردن</button>
      </div>
    `;
  ...
  _renderMonthView(info) {
    ...
    let html = `
      <div class="cbm-calendar-header">
        <button class="cbm-nav-btn" data-action="prevYear">▶</button>
        <div class="cbm-header-title">
          <span class="cbm-title-chip active-view" data-action="showMonth">ماه</span>
          <span class="cbm-title-chip" data-action="showYear">${CbmEngine.toPersianNum(info.year)}</span>
        </div>
        <button class="cbm-nav-btn" data-action="nextYear">◀</button>
      </div>
      <div class="cbm-grid-view">
        ${months.map(m => {
          const classes = ['cbm-grid-cell'];
  ...
  _renderYearView(info) {
    ...
    let html = `
      <div class="cbm-calendar-header">
        <button class="cbm-nav-btn" data-action="prevDecade">▶</button>
        <div class="cbm-header-title">
          <span class="cbm-title-chip" data-action="showMonth">ماه</span>
          <span class="cbm-title-chip active-view" data-action="showYear">${CbmEngine.toPersianNum(years[0].year)}–${CbmEngine.toPersianNum(years[years.length - 1].year)}</span>
        </div>
        <button class="cbm-nav-btn" data-action="nextDecade">◀</button>
      </div>
      <div class="cbm-grid-view">
        ${years.map(y => {
          const classes = ['cbm-grid-cell'];
          ...
          return `<div class="${classes.join(' ')}" data-year="${y.year}">${CbmEngine.toPersianNum(y.year)}</div>`;
        }).join('')}
      </div>
    `;
  ...
  _setHTML(html) {
    ...
    const handle = this.container.querySelector('.cbm-sheet-handle');
    if (handle) {
  ...
  _bindDayEvents() {
    this.container.querySelectorAll('.cbm-day:not(.empty):not(.disabled)').forEach(el => {
    ...
    if (this.engine.rangeMode) {
      this.container.querySelector('.cbm-days').addEventListener('mouseleave', (e) => {
        e.stopPropagation();
        if (!this.engine.rangeStart || this.engine.rangeEnd) return;
  ...
  _updateHoverClasses() {
    ...
    this.container.querySelectorAll('.cbm-day').forEach(el => {
    ...
    // update hint text
    const hint = this.container.querySelector('.cbm-range-hint');
    if (hint) {
  ...
  _bindMonthEvents() {
    this.container.querySelectorAll('.cbm-grid-cell').forEach(el => {
      el.addEventListener('click', (e) => {
        e.stopPropagation();
...
class CbmInputMask {
  ...
  _onInput(e) {
    ...
    let latin = CbmEngine.fromPersianNum(val);
    ...
    const persian = CbmEngine.toPersianNum(formatted).replace(/\//g, '/');
    this.input.value = persian;

    // Try parse complete date
    if (digits.length === 8) {
      const parsed = CbmEngine.parseDateString(persian);
      if (parsed && parsed.jy >= CbmEngine.MIN_YEAR && parsed.jy <= CbmEngine.MAX_YEAR && parsed.jm >= 1 && parsed.jm <= 12) {
  ...
  setValue(jy, jm, jd) {
    this.input.value = CbmEngine.formatPersian(jy, jm, jd);
  }

  setRangeValue(start, end) {
    const s = CbmEngine.formatPersian(start.jy, start.jm, start.jd);
    const e = CbmEngine.formatPersian(end.jy, end.jm, end.jd);
...
/* ================================================================
   CbmDatepicker — Public API (multi-instance + inline support)
   ================================================================
   ...
   Usage — Inline (always visible, no input needed):
     const dp = new CbmDatepicker('#myContainer', {
       inline: true,
       rangeMode: true,
       onChange: (payload) => console.log(payload),
     });
...
class CbmDatepicker {
  constructor(target, options = {}) {
    ...
    this._target = typeof target === 'string'
      ...
      : target;
    if (!this._target) throw new Error(`CbmDatepicker: target not found — "${target}"`);
  ...
  _buildDOM() {
    if (this.options.inline) {
      ...
      this._target.classList.add('cbm-inline-host');
      this._calEl = document.createElement('div');
      this._calEl.className = 'cbm-calendar cbm-inline';
      this._target.appendChild(this._calEl);
      this._renderer = new CbmRenderer(this._calEl, this.engine);
      this._renderer.render();
    } else {
      // Popover: target must be an <input>
      this._input = this._target;
      this._inputMask = new CbmInputMask(this._input, this.engine);

      // Wrap input if not already wrapped
      let anchor = this._input.closest('.cbm-popover-anchor');
      if (!anchor) {
        anchor = document.createElement('div');
        anchor.className = 'cbm-popover-anchor';
      ...
      this._popover.className = 'cbm-calendar-popover';
      this._calEl = document.createElement('div');
      this._calEl.className = 'cbm-calendar';
  ...
  _bindEngineEvents() {
    ...
    this._offRangeStart = engine.on('rangeStart', (payload) => {
      if (!this.options.inline && this._input) {
        const j = payload.jalali || (payload.year !== undefined ? payload : null);
        if (j) this._input.value = CbmEngine.formatPersian(j.year, j.month, j.day) + '  ←  ...';
      }
  ...
  destroy() {
    ...
    if (this.options.inline) {
      ...
      this._target.classList.remove('cbm-inline-host');
    } else {
