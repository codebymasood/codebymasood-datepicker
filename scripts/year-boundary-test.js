...
// Load the library file in a Node environment (no DOM needed for CbmEngine/JalaaliUtil)
const lib = fs.readFileSync(require('path').join(__dirname, '..', 'lib', 'codebymasood-datepicker.js'), 'utf8');

// Evaluate library in current context
// eslint-disable-next-line no-new-func
new Function(lib + '\n;globalThis.__CbmEngine = CbmEngine; globalThis.__JalaaliUtil = JalaaliUtil;')();

const CbmEngine = globalThis.__CbmEngine;
...
function inRangeYear(y) {
  return y >= CbmEngine.MIN_YEAR && y <= CbmEngine.MAX_YEAR;
}

function testPrevYearClamp() {
  const engine = new CbmEngine({ initialYear: 1400, initialMonth: 1 });
  for (let i = 0; i < 5000; i++) engine.goToPrevYear();
  assert(engine.viewYear === CbmEngine.MIN_YEAR, `Expected MIN_YEAR clamp. got ${engine.viewYear}`);
...
function testPrevMonthClamp() {
  ...
  assert(engine.viewYear === CbmEngine.MIN_YEAR, `Expected MIN_YEAR clamp via prevMonth. got ${engine.viewYear}`);
...
function testPrevDecadeClamp() {
  ...
  assert(engine.viewYear === CbmEngine.MIN_YEAR, `Expected MIN_YEAR clamp via prevDecade. got ${engine.viewYear}`);
}

function testGetYearsDoesNotGoBelowMin() {
  const engine = new CbmEngine({ initialYear: CbmEngine.MIN_YEAR, initialMonth: 1 });
  const years = engine.getYears();
  assert(years.length === 12, 'Years grid should always be 12 items');
  assert(years[0].year === CbmEngine.MIN_YEAR, 'Years grid should start at MIN_YEAR at boundary');
...
function testJalaaliUtilThrowsOutsideRange() {
  ...
  let threwHigh = false;
  try { JalaaliUtil.toGregorian(CbmEngine.MAX_YEAR + 1, 1, 1); } catch { threwHigh = true; }
  assert(threwHigh, 'Expected toGregorian to throw for year > MAX_YEAR');
}

function run() {
  console.log('CbmEngine range:', CbmEngine.MIN_YEAR, 'to', CbmEngine.MAX_YEAR);
